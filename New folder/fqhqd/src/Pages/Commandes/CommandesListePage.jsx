...
const ROLE_COLUMNS = {
  client: ["ID", "Commerçant", "Livreur", "Produits", "Total", "Statut", "Date", "Actions"],
  commercant: ["ID", "Client", "Livreur", "Produits", "Total", "Statut", "Date", "Actions"],
  livreur: ["ID", "Client", "Commerçant", "Adresse", "Distance", "Statut", "Date", "Actions"],
}

const CommandesListePage = () => {
  const { data: commandesData, isLoading, isError } = useGetUserCommandes(10000) // Rafraîchir toutes les 10 secondes
  ...
  const acceptCommande = (commandeId) => {
    updateCommandeStatusMutation.mutate({ commandeId, newStatus: "en_preparation" })
  }

  // Fonction pour refuser une commande
  const refuseCommande = (commandeId) => {
    updateCommandeStatusMutation.mutate({ commandeId, newStatus: "refusee" })
  }
  ...
  return (
    <main className="w-full min-h-full bg-gray-100 p-6">
      <h1 className="text-2xl font-bold text-emerald-700 mb-6">Mes Commandes</h1>
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-4 bg-gradient-to-r from-emerald-100 to-emerald-200">
          <h2 className="text-lg font-semibold text-emerald-800">Liste ({commandes.length})</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                {columns.map((col) => (
                  <th key={col} className="py-3 px-4 text-sm font-semibold text-gray-700">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-gray-200">
              {commandes.length === 0 ? (
                <tr>
                  <td colSpan={columns.length} className="py-3 px-4 text-center text-gray-500">
                    Aucune commande
                  </td>
                </tr>
              ) : (
                commandes.map((commande) => (
                  <tr key={commande._id} className="hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">{commande._id.slice(-6)}</td>
                    {renderRoleSpecificColumns(commande, authUser.role)}
                    {authUser.role !== "livreur" && (
                      <>
                        <td className="py-3 px-4">{/* {formatProduits(commande.produits)} */}</td>
                        <td className="py-3 px-4">{commande.total?.toFixed(2)} €</td>
                      </>
                    )}
                    <td className="py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                          STATUS_STYLES[commande.statut] || "bg-gray-100 text-gray-800"
                        }`}
                      >
                        {commande.statut.replace(/_/g, " ")}
                      </span>
                    </td>
                    <td className="py-3 px-4">{new Date(commande.date_creation).toLocaleDateString("fr-FR")}</td>
                    <td className="py-3 px-4 flex gap-2">
                        ...
                        <>
                          ...
                          {authUser.role === "commercant" && (
                            <div className="flex gap-2">
                              <button
                                onClick={() => acceptCommande(commande._id)}
                              ...
                              <button
                                onClick={() => refuseCommande(commande._id)}
                      ...
                      {commande.statut === "en_preparation" && authUser.role === "commercant" && (
                        <a
...
const ErrorMessage = ({ message }) => (
  <div className="flex justify-center items-center h-screen text-red-600">{message}</div>
)
...
const renderRoleSpecificColumns = (commande, role) => {
  switch (role) {
    case "client":
      return (
        <>
          <td className="py-3 px-4">{commande.commercant_id?.nom || "N/A"}</td>
          <td className="py-3 px-4">{commande.livreur_id?.nom || "Non assigné"}</td>
        </>
      )
    case "commercant":
      return (
        <>
          <td className="py-3 px-4">{commande.client_id?.nom || "N/A"}</td>
          <td className="py-3 px-4">{commande.livreur_id?.nom || "Non assigné"}</td>
        </>
      )
    case "livreur":
      return (
        <>
          <td className="py-3 px-4">{commande.client_id?.nom || "N/A"}</td>
          <td className="py-3 px-4">{commande.commercant_id?.nom || "N/A"}</td>
          <td className="py-3 px-4">{commande.adresse_livraison?.rue || "N/A"}</td>
          <td className="py-3 px-4">{calculateDistance(commande.adresse_livraison)}</td>
        </>
      )
    default:
      return null
