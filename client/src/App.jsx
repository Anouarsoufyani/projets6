...
function App() {
  ...
  return (
    <div className={`flex flex-col w-full m-0 h-screen bg-gradient-to-br from-emerald-50 to-teal-100`}>
      {authUser && <Sidebar authUser={authUser} sidebarSize={sidebarSize} navbarHeight={navbarSize} />}
      ...
      >
        <Routes>
          ...
          <Route
            path="/gestion/:role"
            element={authUser && authUser.role === "admin" ? <GestionUsersPage /> : <Navigate to="/login" />}
          />

          <Route
            path="/livreur/:id/pieces"
            element={authUser && authUser.role === "admin" ? <GestionPiecesPage /> : <Navigate to="/login" />}
          />

          <Route path="/uploads/:id/:filename" element={authUser ? <ViewDocs /> : <Navigate to="/login" />} />

          <Route path="/notifications" element={authUser ? <NotificationsPage /> : <Navigate to="/login" />} />

          <Route path="/commande/:id" element={authUser ? <DetailCommande /> : <Navigate to="/login" />} />
        </Routes>
      </div>
      <Toaster />
    </div>
