  ...
  return useMutation({
    mutationFn: async ({ commandeId, livreurId }) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Erreur lors de l'envoi de la demande de livraison")
      }
    ...
    onError: (error) => {
      toast.error(error.message || "Erreur lors de l'envoi de la demande de livraison")
    },
  })
}
