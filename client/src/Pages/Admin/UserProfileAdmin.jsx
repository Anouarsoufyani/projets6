...
import { useGetUserById, useGetDocuments, useGetUserCommandes } from "../../Hooks"
import {
  FaSpinner,
  FaTimesCircle,
  FaCheckCircle,
...
const UserProfileAdmin = () => {
  ...
  const userStats = useMemo(() => {
    ...
    const userCommandes = commandes.filter((cmd) => {
      ...
      if (user.role === "livreur") return cmd.livreur_id === userId || cmd.livreur_id?._id === userId
      return false
    })
  ...
  if (error) {
    return (
      <div className="text-red-600 text-center p-4">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300">
          Retour
        </button>
      </div>
    )
        ...
        for (let i = 0; i < formData.adresses_favorites.length; i++) {
          ...
          if (favorite?.rue && favorite?.ville && favorite?.code_postal) {
            ...
            try {
              ...
              console.log("formData", formData.adresses_favorites)
            } catch (error) {
  ...
  const getStatusIcon = (statut) => {
    switch (statut) {
      case "non vérifié":
        return <FaSpinner className="text-gray-500 animate-spin inline-block mr-2" />
  ...
  const handleDocumentStatusChange = async (documentId, newStatus) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Erreur lors de la mise à jour du statut du document")
      }
      ...
      <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 md:gap-6">
            ...
            />
            <div>
              <p className="text-xl md:text-2xl font-bold text-emerald-700 break-words">{user.nom}</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <FaTrash /> {confirmDelete ? "Confirmer" : "Supprimer"}
            </button>
          </div>
        </div>
            ...
            {edit ? (
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                {user.role === "livreur" && (
                  <>
                    <div className="col-span-1 md:col-span-2">
                      <label className="block mb-1 text-sm font-medium text-gray-700">Véhicules</label>
                      {formData.vehicules && formData.vehicules.length > 0 ? (
                            ...
                            <div key={index} className="border rounded-lg p-3 bg-gray-50">
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  >
                                    {["voiture", "moto", "vélo", "autres"].map((type) => (
                              ...
                              <div className="mt-2">
                                ...
                                <select
                                  ...
                                  className="w-full p-2 border border-gray-300 rounded-md text-sm"
                                >
                                  {["non vérifié", "vérifié", "refusé", "en vérification"].map((statut) => (
                        ...
                        <p className="text-gray-500 text-sm">Aucun véhicule enregistré</p>
                      )}
                      <button
                        type="button"
                        onClick={() =>
                ...
                {user.role === "client" && (
                  <div className="col-span-1 md:col-span-2">
                    ...
                    {formData.adresses_favorites?.map((adresse, index) => (
                      <div key={index} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 mb-2">
                        {["rue", "ville", "code_postal"].map((field) => (
                          <input
                            key={field}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-lg md:text-xl font-bold text-emerald-700 mb-4">Informations</p>
                  <ul className="space-y-2 text-gray-700">
                    {["nom", "email", "numero", "role"].map((field) => (
                      <li key={field} className="break-words">
                        <strong>{field.charAt(0).toUpperCase() + field.slice(1)} :</strong> {user[field] || "N/A"}
                      </li>
                    ))}
                  </ul>
                </div>
                <div>
                  {user.role === "commercant" && (
                    <ul className="space-y-2 text-gray-700">
                      <li className="break-words">
                        <strong>Nom boutique :</strong> {user.nom_boutique || "N/A"}
                      </li>
                      <li className="break-words">
                  ...
                  {user.role === "livreur" && (
                    <>
                      <p className="text-lg font-bold text-emerald-700 mb-2">Véhicules</p>
                        ...
                        <div className="space-y-3">
                          {user.vehicules.map((vehicule, index) => (
                            <div key={index} className="border rounded-lg p-3 bg-gray-50">
                              ...
                              {vehicule.plaque && <p className="text-sm">Plaque: {vehicule.plaque}</p>}
                              {vehicule.couleur && <p className="text-sm">Couleur: {vehicule.couleur}</p>}
                      ...
                      <ul className="space-y-2 text-gray-700 mt-4">
                        <li className="break-words">
                          <strong>Disponibilité :</strong> {user.disponibilite ? "Oui" : "Non"}
                        </li>
                        <li className="break-words">
                          <strong>Distance max :</strong> {user.distance_max} km
                        </li>
                      </ul>
                    </>
                  )}
                  {user.role === "client" && (
                    <ul className="space-y-2 text-gray-700">
                      <li>
                        ...
                        {user.adresses_favorites?.length > 0 ? (
                          <ul className="ml-4 list-disc">
                            {user.adresses_favorites.map((adresse, index) => (
                              <li key={index} className="break-words">
                                {adresse.rue} - {adresse.ville}, {adresse.code_postal}
                              </li>
                            ))}
                          </ul>
                        ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-emerald-500">
                ...
                <div className="text-2xl font-bold text-emerald-700">{userStats.totalCommandes}</div>
              ...
              <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-blue-500">
                <div className="flex items-center justify-between pb-4">
                  <h3 className="text-base font-semibold text-gray-800">
                    {user.role === "livreur" ? "Livraisons" : "Revenus"}
                  </h3>
                ...
                <div className="text-2xl font-bold text-blue-700">
                  {user.role === "livreur" ? userStats.totalRevenu : formatCurrency(userStats.totalRevenu)}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {timeRange === "week" ? "Cette semaine" : timeRange === "month" ? "Ce mois" : "Cette année"}
                </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-purple-500">
                <div className="flex items-center justify-between pb-4">
                  <h3 className="text-base font-semibold text-gray-800">
                    {user.role === "livreur" ? "Note moyenne" : "Taux de conversion"}
                  </h3>
                ...
                <div className="text-2xl font-bold text-purple-700">
                  {user.role === "livreur" ? user.note_moyenne || "N/A" : `${userStats.tauxConversion}%`}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {user.role === "livreur" ? `${user.nombre_livraisons || 0} livraisons` : "Commandes livrées / Total"}
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-4 mt-6">
                ...
                <div className="h-64 border border-gray-100 rounded-lg p-4 bg-gray-50">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={userStats.commandesParStatut}
                        cx="50%"
                        cy="50%"
                        labelLine={false}
                        outerRadius={80}
                      ...
                      >
                        {userStats.commandesParStatut.map((entry, index) => (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50">
                        <th className="text-left py-3 px-4 font-semibold text-gray-600 rounded-tl-lg">ID</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Date</th>
                        <th className="text-left py-3 px-4 font-semibold text-gray-600">Statut</th>
                        <th className="text-right py-3 px-4 font-semibold text-gray-600 rounded-tr-lg">Montant</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {userStats.commandesRecentes.map((cmd) => (
                        <tr key={cmd._id} className="hover:bg-emerald-50 transition-colors duration-150">
                          <td className="py-3 px-4 font-medium text-emerald-600">#{cmd._id.slice(-6)}</td>
                          <td className="py-3 px-4">{new Date(cmd.date_creation).toLocaleDateString()}</td>
                          <td className="py-3 px-4">
                            <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                              {cmd.statut.replace(/_/g, " ")}
                            </span>
                          </td>
                          <td className="py-3 px-4 text-right font-semibold">{formatCurrency(cmd.total)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                {documents.data.map((doc) => (
                  <div key={doc._id} className="border rounded-lg p-3 bg-gray-50">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      Label: <span className="font-semibold">{doc.label}</span>
                    </p>
                    <p className="text-sm text-gray-600 mb-2">
                      Statut:{" "}
                    ...
                    {doc.statut === "en_attente" && (
                      <div className="mt-2 flex gap-2">
                        <button
                          onClick={() => handleDocumentStatusChange(doc._id, "validé")}
                          className="px-2 py-1 bg-green-500 text-white text-xs rounded hover:bg-green-600"
                        >
                          Valider
                        </button>
                        <button
                          onClick={() => handleDocumentStatusChange(doc._id, "refusé")}
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            ...
            {actionType === "en vérification" && (
              <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md text-red-600 text-sm">
                <strong>Attention :</strong> Si un véhicule actuellement vérifié n'est pas sélectionné, il passera
                automatiquement au statut "refusé".
              </div>
            )}
            ...
            {user?.vehicules && user.vehicules.length > 0 ? (
              <div className="space-y-3 max-h-60 overflow-y-auto mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    <label htmlFor="select-all" className="font-medium cursor-pointer flex items-center">
                      <input
                        ...
                        onChange={() => setSelectAllVehicles(!selectAllVehicles)}
                ...
                {user.vehicules.map((vehicule, index) => (
                  <div
                    ...
                    } ${actionType === "en vérification" && vehicule.statut === "vérifié" && !selectedVehicles.includes(index) ? "border-red-300" : ""}`}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        {vehicule.plaque && <p className="text-sm">Plaque: {vehicule.plaque}</p>}
                        {vehicule.couleur && <p className="text-sm">Couleur: {vehicule.couleur}</p>}
                        <p className="text-xs text-gray-500 mt-1">Statut actuel: {vehicule.statut}</p>
                        {actionType === "en vérification" &&
                          vehicule.statut === "vérifié" &&
                          !selectedVehicles.includes(index) && (
                            <p className="text-xs text-red-500 mt-1 font-medium">
                              Ce véhicule passera en "refusé" si non sélectionné
                            </p>
                          )}
                      </div>
              ...
              <p className="text-gray-500 mb-4">Aucun véhicule disponible</p>
            )}
              ...
              <button
                onClick={() => {
                  ...
                  if (selectedVehicles.length === 0 && user?.vehicules?.length > 0) {
                    ...
                    handleUserStatusChange(actionType, selectedVehicles)
                  }
