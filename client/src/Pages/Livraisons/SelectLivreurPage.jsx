...
const SelectLivreurPage = () => {
    ...
    const { calculateRoute, onMapLoad, clearDirections } =
        useGoogleMapDirections({
        ...
        for (const batch of batches) {
            const promises = batch.map((livreur) => {
                return new Promise((resolve) => {
                    const vehicleType =
                        livreur.vehicule?.type?.toLowerCase() || "autres";
                        ...
                        (result, status) => {
                            if (
                                ...
                                const adjustmentFactor =
                                    getVehicleSpeedFactor(vehicleType);
                                ...
                                if (vehicleType !== "voiture") {
                                    ...
                                    durationValue = Math.round(
                                        durationValue * adjustmentFactor
                                    );
                                ...
                                const estimatedDurationMinutes = Math.round(
                                    (directDistance / speedKmh) * 60
                                );

                                newDistances[livreur._id] = {
                                    distance: `~${directDistance.toFixed(
                                        1
                                    )} km`,
    ...
    const assignLivreurAutomatically = useCallback(async () => {
        ...
        if (autoCriteria.criteria.length === 0) {
            toast.error(
                "Veuillez sélectionner au moins un critère de priorité"
            );
            return;
        }

        // Préparer les données à envoyer
        const criteriaWithOrder = autoCriteria.criteria.map(
            (criterion, index) => ({
                type: criterion,
                order: index + 1,
                enabled: true,
        ...
        try {
            ...
            if (result?.data?.topLivreur) {
                toast.success(
                    `Demande envoyée au livreur ${
                        result.data.topLivreur.nom ||
                        result.data.topLivreur.id.slice(-6)
                    } (score: ${
            ...
            toast.error(
                error.message || "Erreur lors de l'assignation automatique"
            );
        }
    ...
    if (vehiculeFilter.length > 0) {
        filteredLivreurs = filteredLivreurs.filter((livreur) =>
            vehiculeFilter.includes(livreur.vehicule?.type)
        );
    }
                ...
                <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-10">
                    ...
                    >
                        ...
                        <p className="text-center text-gray-600">
                            Choisissez vous-même le livreur parmi les
                            disponibles. Vous pourrez filtrer par distance,
                            note, et type de véhicule.
                        </p>
                    ...
                    >
                        ...
                        <p className="text-center text-gray-600">
                            Laissez le système choisir le meilleur livreur selon
                            vos critères de préférence.
                        </p>
    ...
    if (assignationMode === "auto") {
        return (
            <div className="w-full h-full p-6 flex flex-col">
                <h1 className="text-2xl font-bold text-emerald-700 mb-6">
                    {commandeId
                        ? `Assignation automatique pour la commande #${commandeId.slice(
                              -6
                          )}`
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                <label
                                    className={`flex items-center gap-1 px-3 py-1.5 rounded-md cursor-pointer transition-colors ${
                                        autoCriteria.vehicleTypes.includes(
                                            "voiture"
                                        )
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        className="sr-only"
                                        checked={autoCriteria.vehicleTypes.includes(
                                            "voiture"
                                        )}
                                        onChange={() => {
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                Si aucun type n'est sélectionné, tous les types
                                seront considérés.
                            </p>
                        </div>
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-gray-700 mb-2">
                                Priorités (cliquez dans l'ordre d'importance):
                            </h3>
                            <div className="flex flex-wrap gap-2 mb-3">
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-3 py-1.5 rounded-md transition-colors relative ${
                                        autoCriteria.criteria.includes(
                                            "distance"
                                        )
                            ...
                            {autoCriteria.criteria.length > 0 && (
                                <div className="mt-2 p-2 bg-gray-50 rounded-md border border-gray-200">
                                    ...
                                    <p className="text-xs text-gray-500 mt-2">
                                        Le système assignera un livreur en
                                        priorisant les critères dans cet ordre.
                                    </p>
                                </div>
                            )}
                        </div>
    ...
    const handleSelectLivreur = (livreur) => {
        ...
        if (livreurDistances[livreur._id]?.route) {
            ...
            directionsRendererRef.current.setDirections(
                livreurDistances[livreur._id].route
            );
        ...
        filteredLivreurs = [...filteredLivreurs].sort((a, b) => {
            ...
            for (const criterion of sortCriteria) {
                if (criterion === "distance") {
                    const distA =
                        livreurDistances[a._id]?.distanceValue ||
                        Number.POSITIVE_INFINITY;
                    const distB =
                        livreurDistances[b._id]?.distanceValue ||
                        Number.POSITIVE_INFINITY;
                    if (distA !== distB) return distA - distB;
                } else if (criterion === "duration") {
                    const durA =
                        livreurDistances[a._id]?.durationValue ||
                        Number.POSITIVE_INFINITY;
                    const durB =
                        livreurDistances[b._id]?.durationValue ||
                        Number.POSITIVE_INFINITY;
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1 text-sm bg-white px-2 py-1 rounded-full">
                                ...
                                <span>
                                    {livreurDistances[selectedLivreur._id]
                                        ?.distance || "Calcul..."}
                                </span>
                            </div>
                        </div>
                ...
                <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-md">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="bg-white px-2 py-1 rounded-full text-blue-700 font-medium">
                                {distance}
                            </span>
                        </div>
                        <div className="flex items-center gap-2">
                                ...
                                <div className="p-3 max-w-xs">
                                    ...
                                    <div className="space-y-2">
                                        <p className="flex items-center gap-2 text-sm">
                                            {getVehicleIcon(
                                                selectedMarker.vehicule?.type
                                            )}
                                            <span className="text-gray-700">
                                                <span className="font-medium">
                                                    Véhicule:
                                                </span>{" "}
                                                {selectedMarker.vehicule
                                                    ?.type || "N/A"}
                                                {selectedMarker.vehicule
                                                    ?.modele &&
                                                    ` (${selectedMarker.vehicule.modele})`}
                                            </span>
                                        </p>
                                        ...
                                        <p className="flex items-center gap-2 text-sm">
                                            ...
                                            <span className="text-gray-700">
                                                ...
                                                {selectedMarker.note_moyenne.toFixed(
                                                    1
                                                )}
                                                /5
                                            </span>
                ...
                <div className="md:w-5/12 w-full bg-white p-4 rounded-lg shadow-md overflow-auto">
                    ...
                    {showFilters && (
                        <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            ...
                            <div className="mb-3">
                                <h3 className="text-sm font-medium text-gray-700 mb-2">
                                    Trier par (combinable):
                                </h3>
                                <div className="flex flex-wrap gap-2">
                                    <button
                                        onClick={() =>
                                            toggleSortCriterion("distance")
                                        }
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    >
                                        <input
                                            type="checkbox"
                                            className="sr-only"
                                            checked={vehiculeFilter.includes(
                                                "voiture"
                                            )}
                                            onChange={() => {
                                                if (
                                                    vehiculeFilter.includes(
                                                        "voiture"
                                                    )
                                                ) {
                                                    setVehiculeFilter(
                                    ...
                                    >
                                        ...
                                        <td className="py-4 px-4">
                                            <div className="flex flex-col">
                                                {livreur.vehicules &&
                                                livreur.vehicules.length > 0 ? (
                                                    (() => {
                                                        const currentVehicle =
                                                            livreur.vehicules.find(
                                                                (v) => v.current
                                                            );
                                                        if (currentVehicle) {
                                                            return (
