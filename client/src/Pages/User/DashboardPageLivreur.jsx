      ...
      {authUser?.statut !== "vérifié" ? (
        ...
        <>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <h1 className="text-2xl font-bold text-emerald-700">Bienvenue {authUser.nom}</h1>
            ...
            <div className="bg-white px-4 py-2 rounded-lg shadow-md flex items-center gap-3 mt-2 md:mt-0">
              ...
              <div>
                <div className="flex items-center">
                  <span className="text-xl font-bold text-gray-800 mr-2">{averageRating}</span>
                  <StarRating rating={Math.round(averageRating)} />
                </div>
                <p className="text-xs text-gray-500">
                  {reviews?.length || 0} avis client{reviews?.length !== 1 ? "s" : ""}
                </p>
              </div>
            </div>
          </div>

          <div className={`flex flex-col gap-4 bg-white p-6 rounded-lg shadow-lg mb-6`}>
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg">
              ...
              <div className="space-y-4">
                {reviews.slice(0, 3).map((review) => (
                  <div key={review._id} className="bg-gray-50 p-4 rounded-lg border-l-4 border-emerald-400">
