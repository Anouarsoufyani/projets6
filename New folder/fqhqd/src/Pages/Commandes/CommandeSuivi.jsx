...
const CommandeSuivi = () => {
  ...
  // Récupérer les données de la commande avec un rafraîchissement automatique
  ...
  const validateCommercantMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["getCommande", id])
  ...
  // Avoir la geolocalisation exacte du client grâce à l'adresse
  ...
  const { data: coordsClient, isLoading: isLoadingCoordsClient } = useGetCoords(adresseClient)

  // Avoir la geolocalisation exacte du commerçant grâce à l'adresse
  ...
  useEffect(() => {
    // Déterminer les coordonnées à utiliser en fonction du statut
    const deliveryStatus = livreurStatus?.status || commande?.data?.statut || "en_attente"

    if (!mapInitialized) {
      if (deliveryStatus === "recuperee_par_livreur" || deliveryStatus === "en_livraison") {
        // Si la commande est récupérée, centrer sur les coordonnées du client
        if (coordsClient && coordsClient.lat && coordsClient.lng) {
          setMapCenter({ lat: coordsClient.lat, lng: coordsClient.lng })
          setMapInitialized(true)
        }
      } else {
        // Sinon, centrer sur les coordonnées du commerçant
        if (coordsCommercant && coordsCommercant.lat && coordsCommercant.lng) {
          setMapCenter({ lat: coordsCommercant.lat, lng: coordsCommercant.lng })
          setMapInitialized(true)
        }
      }
    }
  }, [coordsClient, coordsCommercant, mapInitialized, livreurStatus, commande])
  ...
  const calculateRoute = useCallback(async () => {
    if (!currentLivreurPosition || !window.google || !mapRef.current) {
      return
    }

    // Déterminer la destination en fonction du statut de la commande
    const deliveryStatus = livreurStatus?.status || commande?.data?.statut || "en_attente"
    let destination = null

    if (deliveryStatus === "recuperee_par_livreur" || deliveryStatus === "en_livraison") {
      // Si la commande est récupérée, la destination est l'adresse du client
      if (coordsClient && coordsClient.lat && coordsClient.lng) {
        destination = { lat: coordsClient.lat, lng: coordsClient.lng }
      }
    } else {
      // Sinon, la destination est l'adresse du commerçant
      if (coordsCommercant && coordsCommercant.lat && coordsCommercant.lng) {
        destination = { lat: coordsCommercant.lat, lng: coordsCommercant.lng }
      }
    }
  ...
  }, [currentLivreurPosition, coordsClient, coordsCommercant, livreurStatus, commande])

  // Mettre à jour l'itinéraire lorsque la position du livreur change ou que le statut change
  useEffect(() => {
    if (currentLivreurPosition && mapRef.current) {
      calculateRoute()
    }
  }, [currentLivreurPosition, calculateRoute, livreurStatus, commande])
  ...
  const handleMapLoad = useCallback(
    (map) => {
      ...
      if (currentLivreurPosition) {
        calculateRoute()
      }
    },
    [calculateRoute, currentLivreurPosition],
  )
  ...
  if (isLoading || isLoadingLivreur || isLoadingCoordsClient || isLoadingCoordsCommercant) {
  ...
  const hasValidCoordinates =
    ...
    ((coordsClient && coordsClient.lat && coordsClient.lng) ||
      (coordsCommercant && coordsCommercant.lat && coordsCommercant.lng))
  ...
  // Fonction pour obtenir le texte du statut formaté
  const getStatusText = (status) => {
    switch (status) {
      ...
      case "prete_a_etre_recuperee":
        return "Prête à être récupérée"
      case "recuperee_par_livreur":
  ...
  const getStatusDescription = (status) => {
    switch (status) {
      case "en_attente":
        return "La commande est en attente de confirmation par le commerçant."
      case "en_preparation":
        return "La commande est en cours de préparation par le commerçant."
      case "prete_a_etre_recuperee":
        return "La commande est prête et attend d'être récupérée par le livreur."
      case "recuperee_par_livreur":
        return "Le livreur a récupéré la commande et est en route vers votre adresse."
      case "en_livraison":
      ...
      case "refusee":
        return "La commande a été refusée par le commerçant."
      default:
        return "Statut inconnu"
    }
  }
      ...
      <div className="flex flex-col lg:flex-row flex-1 gap-6">
        <div className="w-full lg:w-1/3 bg-white p-4 md:p-6 rounded-lg shadow-md">
          ...
          <div className="overflow-auto h-full space-y-6">
            ...
            <div className={`p-4 ${getStatusColor(deliveryStatus)} rounded-lg`}>
              <h3 className="font-medium mb-2">Statut</h3>
              <div className="space-y-2">
                <p className="font-medium">{getStatusText(deliveryStatus)}</p>
                <p className="text-sm text-gray-600">{getStatusDescription(deliveryStatus)}</p>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                      <div
                        className={`h-2.5 rounded-full ${
                          deliveryStatus === "prete_a_etre_recuperee" ? "bg-amber-600" : "bg-blue-600"
                        }`}
                        style={{
        ...
        <div className="w-full lg:w-2/3 shadow-xl rounded-lg overflow-hidden mt-4 lg:mt-0">
          {!hasValidCoordinates ? (
            ...
            >
              ...
              {/* Marqueur pour la destination (commerçant ou client selon le statut) */}
              {deliveryStatus === "recuperee_par_livreur" || deliveryStatus === "en_livraison"
                ? coordsClient && (
                    <Marker
                      position={{
                        lat: coordsClient.lat,
                        lng: coordsClient.lng,
                      }}
                      icon={{
