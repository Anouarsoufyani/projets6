...
import { FaSpinner, FaTimesCircle, FaCheckCircle, FaHourglassHalf } from "react-icons/fa"
...
const GestionUsersPage = () => {
  ...
  return (
    <main className="w-full min-h-full p-6">
      <h1 className="text-2xl font-bold text-emerald-700 mb-6">Gestion {role}s</h1>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                {role === "livreur" && <th className="py-3 px-4">Statut</th>}
            ...
            <tbody className="divide-y divide-gray-200">
              {usersData?.data.map((data) => (
                <tr key={data.id} className="hover:bg-gray-50 transition-colors">
                  <td className="py-3 px-4">{data._id.slice(0, 5)}...</td>
                  ...
                  <td className="py-3 px-4 flex gap-2">
                    ...
                    <button className="text-amber-600 hover:text-amber-800">Modifier</button>
                    <button className="text-red-600 hover:text-red-800">Supprimer</button>
                    {role === "livreur" && data.statut !== "non vérifié" && (
                      <button className="text-purple-600 hover:text-purple-800">
                        <a href={`/livreur/${data._id}/pieces`}>Voir pièces jointes</a>
                      </button>
                    )}
                  </td>
                </tr>
