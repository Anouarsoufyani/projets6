...
import { FaBell, FaCheck, FaTrash, FaStore, FaTruck, FaUser, FaExclamationTriangle } from "react-icons/fa"
      ...
      <div className="w-full h-full p-6 flex flex-col">
        ...
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-md">
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-red-700">Une erreur est survenue lors du chargement des notifications.</p>
              <button
                onClick={() => refetch()}
    ...
    <div className="w-full h-full bg-gradient-to-br from-emerald-50 to-teal-100 p-6 flex flex-col">
          ...
          <div className="flex bg-white rounded-lg shadow-sm overflow-hidden">
            <button
              className={`px-3 py-1.5 text-sm transition-colors ${
                filter === "all" ? "bg-emerald-500 text-white" : "hover:bg-gray-100"
              }`}
            ...
            <button
              className={`px-3 py-1.5 text-sm transition-colors ${
                filter === "unread" ? "bg-emerald-500 text-white" : "hover:bg-gray-100"
              }`}
            ...
            <button
              className={`px-3 py-1.5 text-sm transition-colors ${
                filter === "nouvelle commande" ? "bg-emerald-500 text-white" : "hover:bg-gray-100"
              }`}
            ...
            <button
              className={`px-3 py-1.5 text-sm transition-colors ${
                filter === "livraison" ? "bg-emerald-500 text-white" : "hover:bg-gray-100"
              }`}
      ...
      {filteredNotifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-lg shadow-sm">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Aucune notification</h2>
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between">
                    <p className={`text-sm font-medium ${notification.read ? "text-gray-700" : "text-emerald-700"}`}>
                  ...
                  <p className="text-sm text-gray-800 mt-1 leading-relaxed">
                    {notification.message ? (
                      ...
                      <>
                        Notification de <span className="font-semibold">{notification.sender?.nom ?? "Système"}</span>
                      </>
                    )}
                    <br />
                    {/* {(notification.type ===
                    ...
                    {notification.isRequest &&
                      (notification.isAccepted && notification.commande_id.livreur_id == authUser._id ? (
                        <>
                          <p className="mt-2 text-green-600">Vous avez accepté la commande</p>
                          <Link
                        ...
                        <p className="mt-2 text-red-600">Vous avez refusé la commande</p>
                      ) : notification.commande_id && notification.commande_id.livreur_id == authUser._id ? (
                        <>
                          <p className="mt-2 text-green-600">Vous avez accepté la commande</p>
                          <Link
                            to={`/livraison/${notification.commande_id._id}`}
                      ...
                      ) : notification.commande_id && notification.commande_id.livreur_id ? (
                        <p className="mt-2 text-gray-600">La commande a déjà un livreur assigné</p>
                      ) : (
                        <div className="mt-2 flex gap-2">
                          <button
                            className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded"
                            onClick={() => {
                              assignLivreur(notification.commande_id, authUser._id, notification._id, "accepter")
                            }}
                          >
                            Accepter
                          </button>
                          <button
                            ...
                            onClick={() => {
                              assignLivreur(notification.commande_id, authUser._id, notification._id, "refuser")
                            }}
                          >
                            Refuser
                          </button>
                  ...
                  <div className="mt-2 flex justify-between items-center">
                    <span className="text-xs text-gray-500">{getFormattedDate(notification.createdAt)}</span>
